...
def requirement1(host, user, password, directory):
    ...
    f_cities = open('./cities.csv', 'r', encoding='utf-8')
    next(f_cities)

    while True:
        line = f_cities.readline() #cities csv를 한 줄 읽음.
        line_list = line.replace('\n', '') #줄바꿈을 위해 존재하는 '\n'을 없앰
        line_list = line_list.split(',') #콤마를 기준으로 쪼갠다
        for i, j in enumerate(line_list): #리스트에 들어 있는 애들을 (index, item)의 형태로 반환해주는 게 enumerate 함수
            if line_list[i] == '':
                line_list[i] = None #빈 문자열(NULL값)의 경우 NONE으로 바꾼다
            try:
                line_list[i] = int(j) #이 try-except구문은 int로 string을 바꾸려 시도한 다음 string안에 들어 있는 게 int로 반환될 수 있는
                                        #숫자여야지만 바뀜. int로 바뀔 수 없는 무언가라면 에러 발생.
            except:
                pass #에러가 발생할 경우 그냥 보내준다.  (int로 바뀔 수 없는 VARCHAR 등의 data type들)
        if line: #line이 비어있지 않다면 실행하는 구문
            cursor.execute(sql_cities, line_list) #cities TABLE에 cities csv에서 읽어온 한 줄 한 줄을 리스트화한 것을 삽입함.
        else:
            break #while loop을 끝냄

    #나머지 table 삽입 과정도 동일하므로 주석 생략했습니다

    cnx.commit() #지금까지 한 것 적용
    f_cities.close() #cities csv연 것 close


    #gatherings TABLE에 VALUE 삽입
    sql_gatherings = """INSERT INTO gatherings VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
    f_gatherings = open('./gatherings.csv', 'r', encoding='utf-8')
    next(f_gatherings) #csv의 첫 줄은 column이름들이므로 한 줄 건너뛰어서 data가 존재하는 줄부터 시작.
    ...
    f_meetings = open('./meetings.csv', 'r', encoding='utf-8')
    next(f_meetings)
    ...
    f_members = open('./members.csv', 'r', encoding='utf-8')
    next(f_members)
    ...
    f_member_gathering = open('./member_gathering.csv', 'r', encoding='utf-8')
    ...
    f_organizers = open('./organizers.csv', 'r', encoding='utf-8')
    next(f_organizers)
    ...
    f_topics = open('./topics.csv', 'r', encoding='utf-8')
    next(f_topics)
    ...
    f_meeting_places = open('./meeting_places.csv', 'r', encoding='utf-8')
    ...
    while True:
        ...
        for i, j in enumerate(line_list):
            ...
            #다른 TABLE들의 코드와 다른데, meeting_places의 zip이 float64로 나오는 경우를 함께 처리하기 위해
            #'1006.0'과 같은 string-은 int로 한 번에 변환이 어려워 float으로 먼저 변환하고 int로 다시 바꿔주는 과정 거침
...
def requirement2(host, user, password):
    ...
    cursor.execute('''
        SELECT workson3.Year, NewUser, NewGroup, NewMeeting
        FROM (SELECT workson1.Year as Year, NewUser, NewMeeting
        FROM (SELECT year(joined) as Year, COUNT(*) as NewUser
        FROM member_gathering
        GROUP BY year(joined)) as workson1 
        LEFT JOIN (SELECT year(created) as Year, COUNT(*) as NewMeeting
        FROM meetings
        GROUP BY year(created)) as workson2
        ON workson1.Year = workson2.Year) as workson3
        LEFT JOIN (SELECT year(created) as Year, COUNT(*) as NewGroup
        FROM gatherings
        GROUP BY year(created)) as workson4
        ON workson3.Year = workson4.Year
        ORDER BY workson3.Year;
        ''')

    #req2의 결과를 저장하기 위한 text file open
    fopen = open('project2_team%02d_req2.txt' % team, 'w', encoding='utf8')

    rows = cursor.fetchall() #cursor에 저장된 모든 데이터들을 가져와서
    for line in rows: #한 줄씩 읽으며 txt파일에 추가하는 과정
        for i in range(len(line)):
            fopen.write(str(line[i]))
            if i < len(line) - 1: fopen.write(';')
        if line != rows[len(rows) - 1]: fopen.write('\n')

    fopen.close() #txt 파일 닫음
    cursor.close()
...
def requirement3(host, user, password):
    ...
    cursor.execute("""
            ...
            INNER JOIN (SELECT meeting_place_id as id
            FROM meetings
            GROUP BY meeting_place_id
            HAVING COUNT(*)<=3) as view2
            ON view1.id=view2.id) as view3
...
user = 'user'
password = 'password'
directory_in = 'C:/directory/of/data/csv_files'
